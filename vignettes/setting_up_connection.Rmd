...
Once secret credentials have been saved locally, they will be available for generating the API token.  A convenience function is provided to allow for generation of a token, which needs to be done periodically without having to directly input the secret or email address associated with the secret provided these have been saved locally.  Alternatively, these arguments can be provided to the function but this is a lot less safe as it is stored to your local R history file.
